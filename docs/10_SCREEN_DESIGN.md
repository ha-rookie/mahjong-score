...
### Active Session終了操作
- スコア表・精算欄の後に操作を配置する
- Game 1件以上では「Sessionを終了」を表示し、Results確認後に終了を確定する
- Game 0件では「Sessionを取り消す」を表示し、通常終了へ進ませない
- 取り消し確認では、Sessionの参加者・チップ・メモが削除され履歴へ残らないことを明示する
- 誤操作防止の確認を行う
- 終了または取り消し成功後はHomeへ戻し、「今日の麻雀を始める」を再表示する
