...
function App() {
  ...
  const openPerformance=async()=>{if(!group)return;setIsBusy(true);setErrorMessage(null);const r=await services.getPlayerPerformanceAggregates.execute(group.id);if(!r.ok)setErrorMessage(r.error.userMessage??"通算成績を読み込めませんでした。");else{setPerformance(r.value);setView("performance");}setIsBusy(false);};
