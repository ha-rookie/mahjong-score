...
function App() {
  ...
  const loadPerformance=async(period=performancePeriod,year=performanceYear,month=performanceMonth)=>{if(!group)return;setIsBusy(true);setErrorMessage(null);try{const params=new URLSearchParams();if(period!=="all"){params.set("year",String(year));if(period==="month")params.set("month",String(month));}const response=await fetch(`/api/groups/${encodeURIComponent(group.id)}/performance-summary${params.size?"?"+params.toString():""}`);if(!response.ok)throw new Error("performance");const payload=await response.json() as {performance:PlayerPerformanceAggregate[]};setPerformance(payload.performance);setView("performance");}catch{setErrorMessage("通算成績を読み込めませんでした。");}setIsBusy(false);};
  const openPerformance=async()=>{setPerformancePeriod("all");await loadPerformance("all");};
  const openHistory=async()=>{if(!group)return;setIsBusy(true);setErrorMessage(null);try{const response=await fetch(`/api/groups/${encodeURIComponent(group.id)}/history-summary`);if(!response.ok)throw new Error("history");const payload=await response.json() as {sessions:Array<Session&{gameCount:number}>};setHistory(payload.sessions);setHistoryGameCounts(Object.fromEntries(payload.sessions.map(s=>[s.id,s.gameCount])));setView("history");}catch{setErrorMessage("過去の麻雀を読み込めませんでした。");}setIsBusy(false);};
