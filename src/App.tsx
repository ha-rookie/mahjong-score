  ...
  return <div className="app-shell">
    ...
    <main className="page">
      ...
      activeSession?<section className="score-session">
        <div className="score-session__heading"><div><p className="screen-eyebrow">SCORE SHEET</p><h1>{activeSession.session.sessionDate}</h1><p className="score-session__meta">{getModeLabel(participantIds.length)} · {games.length}半荘</p></div><button className="session-refresh-button" type="button" aria-label="最新の状態に更新" title="最新の状態に更新" disabled={isBusy||isSessionRefreshing} onClick={requestSessionRefresh}><svg className={isSessionRefreshing?"session-refresh-icon session-refresh-icon--spinning":"session-refresh-icon"} viewBox="0 0 24 24" aria-hidden="true"><path d="M20 11a8 8 0 1 0-2.34 5.66"/><path d="M20 4v7h-7"/></svg><span>{isSessionRefreshing?"更新中":"更新"}</span></button></div>
