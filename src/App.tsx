...
function App() {
  ...
  const openHistory=async()=>{if(!group)return;setView("history");setIsHistoryLoading(true);setErrorMessage(null);try{const response=await fetch(`/api/groups/${encodeURIComponent(group.id)}/history-summary`);if(!response.ok)throw new Error("history");const payload=await response.json() as {sessions:Array<Session&{gameCount:number}>};setHistory(payload.sessions);setHistoryGameCounts(Object.fromEntries(payload.sessions.map(s=>[s.id,s.gameCount])));setView("history");}catch{setErrorMessage("過去の麻雀を読み込めませんでした。");}setIsHistoryLoading(false);};
