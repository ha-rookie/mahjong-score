...
import type { Game, GameTagType, Group, Player, PlayerId, Session } from "./domain";
...
type View = "home" | "session-setup" | "results" | "history";
...
function App() {
  ...
  return <div className="app-shell">
    ...
    <main className="page">
      ...
      <><section className="home-hero"><p className="screen-eyebrow">HOME</p><h1>仲間との麻雀を、静かに記録する。</h1><p className="home-hero__meta">{group.name} · {players.length}人登録</p>{players.length>=3?<Button block onClick={openSessionSetup}>今日の麻雀を始める</Button>:<p className="empty-hint">Sessionを始めるには、メンバーを3人以上登録してください。</p>}<Button block variant="secondary" disabled={isBusy} onClick={()=>void openHistory()}>過去の麻雀を見る</Button></section>
