...
export class ApiGroupRepository implements GroupRepository { constructor(private readonly api:WorkerApiClient){} async list(){const r=await this.api.request<{groups:Group[]}>("/api/groups");return r.ok?ok(r.value.groups):r;} async findById(id:GroupId):Promise<Result<Group|null>>{const r=await this.list();return r.ok?ok(r.value.find(x=>x.id===id)??null):r;} async save(group:Group):Promise<Result<void>>{const existing=await this.findById(group.id);if(!existing.ok)return existing;const path=existing.value?`/api/groups/${encodeURIComponent(group.id)}`:"/api/groups";const r=await this.api.request<{group:Group}>(path,{method:existing.value?"PATCH":"POST",body:JSON.stringify(group)});return r.ok?ok(undefined):r;} }
