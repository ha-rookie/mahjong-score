...
export class ApiSessionRepository implements SessionRepository{
  ...
  async save(s:Session){const r=await this.api.request<unknown>(`/api/sessions/${encodeURIComponent(s.id)}`,{method:"PATCH",body:JSON.stringify({...s,expectedVersion:s.version,updatedAt:new Date().toISOString()})});return r.ok?ok(undefined):r;}
  async remove(id:SessionId,expectedVersion?:number):Promise<Result<void>>{const suffix=expectedVersion===undefined?"":`?version=${expectedVersion}`;const r=await this.api.request<unknown>(`/api/sessions/${encodeURIComponent(id)}${suffix}`,{method:"DELETE"});return r.ok?ok(undefined):r;}
  async cancelEmpty(id:SessionId,expectedVersion?:number):Promise<Result<void>>{const r=await this.api.request<unknown>(`/api/sessions/${encodeURIComponent(id)}/cancel`,{method:"POST",body:JSON.stringify({expectedVersion})});return r.ok?ok(undefined):r;}
