...
export default { async fetch(request:Request,env:Env):Promise<Response>{
 ...
 const m=url.pathname.match(/^\/api\/groups\/([^/]+)\/players$/);if(request.method==="GET"&&m){const groupId=decodeURIComponent(m[1]);if(!await canUseGroup(env,authUserId,groupId))return bad("forbidden","Group access required",403);const r=await env.DB.prepare("SELECT p.id,p.display_name AS displayName,p.created_at AS createdAt,p.updated_at AS updatedAt,gp.user_id AS userId FROM players p JOIN group_players gp ON gp.player_id=p.id WHERE gp.group_id=? AND gp.active=1 ORDER BY p.created_at,p.id").bind(groupId).all();return json({players:r.results});}
 ...
 const manage=url.pathname.match(/^\/api\/admin\/groups\/([^/]+)\/users\/([^/]+)$/);if(request.method==="PATCH"&&manage){if(!await isSystemAdmin(env,authUserId))return bad("forbidden","System admin role required",403);const groupId=decodeURIComponent(manage[1]),userId=decodeURIComponent(manage[2]),b=await body(request),role=b?.role==="group_admin"||b?.role==="member"?b.role:null,playerId=b?.playerId===null?null:textValue(b?.playerId),now=new Date().toISOString();if(!role)return bad("invalid_role","role must be group_admin or member");const groupExists=await env.DB.prepare("SELECT 1 AS ok FROM groups WHERE id=?").bind(groupId).first();const userExists=await env.DB.prepare("SELECT 1 AS ok FROM users WHERE id=?").bind(userId).first();if(!groupExists||!userExists)return bad("not_found","Group or User not found",404);if(playerId){const playerExists=await env.DB.prepare("SELECT 1 AS ok FROM group_players WHERE group_id=? AND player_id=? AND active=1").bind(groupId,playerId).first();if(!playerExists)return bad("invalid_player_link","Player is not active in this Group",409);}try{const statements=[env.DB.prepare("INSERT INTO group_memberships(group_id,user_id,role,created_at,updated_at) VALUES(?,?,?,?,?) ON CONFLICT(group_id,user_id) DO UPDATE SET role=excluded.role,updated_at=excluded.updated_at").bind(groupId,userId,role,now,now),env.DB.prepare("UPDATE group_players SET user_id=NULL WHERE group_id=? AND user_id=?").bind(groupId,userId)];if(playerId)statements.push(env.DB.prepare("UPDATE group_players SET user_id=? WHERE group_id=? AND player_id=? AND user_id IS NULL").bind(userId,groupId,playerId));const rs=await env.DB.batch(statements);if(playerId&&rs[2]?.meta.changes!==1)throw new Error("player link");return json({ok:true,groupId,userId,role,playerId});}catch{return bad("membership_update_failed","Membership or Player link could not be updated",409);}}
