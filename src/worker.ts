...
export default { async fetch(request:Request,env:Env):Promise<Response>{
 ...
 if(request.method==="POST"&&cancelSession){
   const id=decodeURIComponent(cancelSession[1]),sg=await sessionGroup(env,id);
   if(!sg||!await canUseGroup(env,authUserId,sg.groupId))return deny("Group access required");
   const b=await body(request),expectedVersion=typeof b?.expectedVersion==="number"&&Number.isInteger(b.expectedVersion)&&b.expectedVersion>0?b.expectedVersion:null;
   if(expectedVersion===null)return bad("invalid_expected_version","expectedVersion is required");
   const current=await env.DB.prepare("SELECT s.status,s.version,(SELECT COUNT(*) FROM games g WHERE g.session_id=s.id) AS gameCount FROM sessions s WHERE s.id=?").bind(id).first<{status:string;version:number;gameCount:number}>();
   if(!current)return bad("session_not_found","Session not found",404);
   if(current.status!=="active")return bad("session_not_active","Finalized Session cannot be cancelled",409);
   if(Number(current.gameCount)!==0)return bad("session_not_empty","Session with Games cannot be cancelled",409);
   const deleted=await env.DB.prepare("DELETE FROM sessions WHERE id=? AND version=? AND status='active' AND NOT EXISTS(SELECT 1 FROM games WHERE session_id=?)").bind(id,expectedVersion,id).run();
   if((deleted.meta.changes??0)===0)return bad("stale_update","Session was updated by another client",409);
   auditSuccess("empty_session_cancelled",{groupId:sg.groupId,resourceType:"session",resourceId:id});
   return new Response(null,{status:204});
 }
 const details=url.pathname.match(/^\/api\/sessions\/([^/]+)$/);if(request.method==="GET"&&details){const id=decodeURIComponent(details[1]),sg=await sessionGroup(env,id);if(!sg||!await canUseGroup(env,authUserId,sg.groupId))return deny("Group access required");const s=await env.DB.prepare("SELECT id,group_id AS groupId,session_date AS sessionDate,started_at AS startedAt,ended_at AS endedAt,status,note,version FROM sessions WHERE id=?").bind(id).first();if(!s)return bad("session_not_found","Session not found",404);const n=await env.DB.prepare("SELECT player_id AS playerId,note FROM session_participant_notes WHERE session_id=? ORDER BY player_id").bind(id).all();const c=await env.DB.prepare("SELECT player_id AS playerId,chip_count AS chipCount FROM chip_results WHERE session_id=? ORDER BY player_id").bind(id).all();return json({session:{...s,participantNotes:n.results,chipResults:c.results}});}if(request.method==="PATCH"&&details){const id=decodeURIComponent(details[1]),sg=await sessionGroup(env,id);if(!sg||!await canUseGroup(env,authUserId,sg.groupId))return deny("Group access required");const currentSession=await env.DB.prepare("SELECT s.status,(SELECT COUNT(*) FROM games g WHERE g.session_id=s.id) AS gameCount FROM sessions s WHERE s.id=?").bind(id).first<{status:string;gameCount:number}>();if(!currentSession)return bad("session_not_found","Session not found",404);if(currentSession.status==="finalized")return bad("session_not_active","Finalized Session is read-only",409);const b=await body(request),note=b?.note===null?null:typeof b?.note==="string"?b.note:null,status=b?.status==="finalized"?"finalized":"active",endedAt=b?.endedAt===null?null:textValue(b?.endedAt),updatedAt=textValue(b?.updatedAt),expectedVersion=typeof b?.expectedVersion==="number"&&Number.isInteger(b.expectedVersion)&&b.expectedVersion>0?b.expectedVersion:null;if(!updatedAt||expectedVersion===null)return bad("invalid_session_update","updatedAt and expectedVersion are required");if(status==="finalized"&&Number(currentSession.gameCount)===0)return bad("session_empty","Session must contain at least one Game before finalization",409);const participantNotes=Array.isArray(b?.participantNotes)?b.participantNotes:[],chipResults=Array.isArray(b?.chipResults)?b.chipResults:[];const notes=participantNotes.map(v=>{const o=v&&typeof v==="object"?v as Record<string,unknown>:{};return {playerId:textValue(o.playerId),note:typeof o.note==="string"?o.note:null}}),chips=chipResults.map(v=>{const o=v&&typeof v==="object"?v as Record<string,unknown>:{};return {playerId:textValue(o.playerId),chipCount:typeof o.chipCount==="number"&&Number.isInteger(o.chipCount)?o.chipCount:null}});if(notes.some(v=>!v.playerId||v.note===null)||chips.some(v=>!v.playerId||v.chipCount===null)||new Set(notes.map(v=>v.playerId)).size!==notes.length||new Set(chips.map(v=>v.playerId)).size!==chips.length||chips.reduce((n,v)=>n+(v.chipCount??0),0)!==0)return bad("invalid_session_details","Participant notes/chips are invalid");const participantRows=await env.DB.prepare("SELECT DISTINCT sp.player_id AS playerId FROM participant_segments ps JOIN segment_players sp ON sp.segment_id=ps.id WHERE ps.session_id=?").bind(id).all();const participantIds=(participantRows.results as Array<{playerId:string}>).map(v=>v.playerId);if(!participantIds.length||notes.some(v=>!participantIds.includes(v.playerId as string))||chips.some(v=>!participantIds.includes(v.playerId as string))||(chips.length>0&&(chips.length!==participantIds.length||participantIds.some(playerId=>!chips.some(v=>v.playerId===playerId)))))return bad("invalid_session_participants","Participant notes/chips must belong to the Session and chip results must cover all participants");try{const updateSession=status==="finalized"?env.DB.prepare("UPDATE sessions SET note=?,status=?,ended_at=?,updated_at=?,version=version+1 WHERE id=? AND version=? AND EXISTS(SELECT 1 FROM games WHERE session_id=?)").bind(note,status,endedAt,updatedAt,id,expectedVersion,id):env.DB.prepare("UPDATE sessions SET note=?,status=?,ended_at=?,updated_at=?,version=version+1 WHERE id=? AND version=?").bind(note,status,endedAt,updatedAt,id,expectedVersion);const rs=await env.DB.batch([env.DB.prepare("DELETE FROM session_participant_notes WHERE session_id=? AND EXISTS(SELECT 1 FROM sessions WHERE id=? AND version=?)").bind(id,id,expectedVersion),...notes.map(v=>env.DB.prepare("INSERT INTO session_participant_notes(session_id,player_id,note) SELECT ?,?,? WHERE EXISTS(SELECT 1 FROM sessions WHERE id=? AND version=?)").bind(id,v.playerId,v.note,id,expectedVersion)),env.DB.prepare("DELETE FROM chip_results WHERE session_id=? AND EXISTS(SELECT 1 FROM sessions WHERE id=? AND version=?)").bind(id,id,expectedVersion),...chips.map(v=>env.DB.prepare("INSERT INTO chip_results(session_id,player_id,chip_count) SELECT ?,?,? WHERE EXISTS(SELECT 1 FROM sessions WHERE id=? AND version=?)").bind(id,v.playerId,v.chipCount,id,expectedVersion)),updateSession]);const updated=rs[rs.length-1]?.meta.changes??0;if(updated!==1)return bad("stale_update","Session was updated by another client",409);return json({ok:true,version:expectedVersion+1});}catch{return bad("session_update_failed","Session could not be updated",409);}}
 ...
 if(gameDetail){
   ...
   if(request.method==="PUT"){
     ...
     if(!segmentId||!playedAt||!Number.isInteger(sequence)||expectedVersion===null||![3,4].includes(parsed.length)||parsed.some(v=>!v.playerId||v.scorePoint===null)||new Set(parsed.map(v=>v.playerId)).size!==parsed.length||parsed.reduce((n,v)=>n+(v.scorePoint??0),0)!==0||parsedTags.some(v=>!v.type))return bad("invalid_game","Game payload is invalid");const segment=await env.DB.prepare("SELECT id FROM participant_segments WHERE id=? AND session_id=?").bind(segmentId,String(base.sessionId)).first();if(!segment)return bad("invalid_game_segment","Segment must belong to the Game Session");const segmentPlayers=await env.DB.prepare("SELECT player_id AS playerId FROM segment_players WHERE segment_id=? ORDER BY seat_order").bind(segmentId).all();const expectedPlayers=(segmentPlayers.results as Array<{playerId:string}>).map(v=>v.playerId),actualPlayers=parsed.map(v=>v.playerId as string);if(![3,4].includes(expectedPlayers.length)||expectedPlayers.length!==actualPlayers.length||expectedPlayers.some(id=>!actualPlayers.includes(id)))return bad("invalid_game_participants","Game results must match the Session participants");
     try{
 ...
 if(segmentDetail){
  ...
  if(request.method==="PUT"){
    return bad("segment_update_not_supported","Session participant composition is fixed after Session start",405);
  }
 }
