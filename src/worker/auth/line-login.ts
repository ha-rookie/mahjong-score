...
const lineFailure=async(response:Response,stage:string)=>{let details:LineError={};try{details=await response.json() as LineError;}catch{details={};}return new Response(JSON.stringify({error:{code:"line_"+stage+"_failed",message:details.error_description??details.error??"LINE request failed",lineStatus:response.status}}),{status:401,headers:{"content-type":"application/json; charset=utf-8"}});};
...
export const bootstrapAdmin=async(request:Request,env:AuthEnv)=>{
  const userId=await authenticatedUserId(request,env);
  if(!userId)return new Response(JSON.stringify({error:{code:"unauthorized",message:"Authentication required"}}),{status:401,headers:{"content-type":"application/json; charset=utf-8"}});
